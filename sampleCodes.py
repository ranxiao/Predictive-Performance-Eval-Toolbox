...
import numpy as np
from core import metrics,scorers, Process, augmenters, utils, mews
...
case_count, case_count_raw = metrics.run(case, case_processor)# case_count dim (#random selection, #pat with triggers, #pat without triggers)
...
for i in range(0,len(thresh)):    
        ...
        np.savez(f'./Performance_timeliness_{i}.npz', TPR=TPR, TNR=TNR, PPV=PPV, WDR=WDR,NPV=NPV, FPR=FPR,FNR=FNR, ACC=ACC, F1=F1)
...
t_lead = 0 # lead time in hours for case condition
t_win =12# prediction horizon in hours for case
...
case_scorers = [scorers.ProportionWarning_case(tlead=t_lead, twin=t_win, tmax=np.inf)]# RanXiao: outside of 12-hour prediction horizon + lead time
control_scorers = [scorers.ProportionWarning(tmin=0, tmax=np.inf)]
...
thresh = np.arange(0, 14) #define threshold or thresholds to be used  
# Define scorer for calculating false positive for case and control separately. for case it's defined as duration outside of prediction horizon, for control it's the whole duration
case_scorers = [scorers.HourlyFalseAlarmRate_case(tlead=t_lead, twin=t_win, tmax=np.inf)]
control_scorers = [scorers.HourlyFalseAlarmRate(tmin=0, tmax=np.inf)]
